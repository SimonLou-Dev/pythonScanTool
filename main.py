...
def main() -> None:
    ...
    network_subparsers = network_parser.add_subparsers(
        dest="network_mode",
        help="Sous-options réseau",
        )

    ## ============== Module Network : PCAP ==============
    pcap_parser = network_subparsers.add_parser(
        "pcap",
        help="Analyse d'un fichier PCAP",
        )
    ...
    iface_parser = network_subparsers.add_parser(
        "iface",
        help="Capture en direct sur une interface réseau",
        )
    ...
    iface_parser.add_argument(
        ...
        "-s",
        action="store_true",
        help="Sauvegarder les paquets capturés dans un fichier PCAP",
        )
...
def add_common_network_options(parser) -> None:
    common_group = parser.add_argument_group("Options communes du module Network")
    common_group.add_argument(
        "--path-bf",
        "-b",
        action="store_true",
        help="Détecter les scans bruteforce d'URLs",
        )
    common_group.add_argument(
        "--http-passwords",
        "-p",
        action="store_true",
        help="Détecter les mots de passe HTTP en clair",
        )
    common_group.add_argument(
        "--http-credentials",
        "-c",
        action="store_true",
        help="Détecter les credentials HTTP en base64",
        )
